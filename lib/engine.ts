...
class RequestSandbox extends uvm {
    constructor () {
        super();
...
engine = RequestSandbox.spawn({
    bootCode: `
        bridge.on('engine', function (data) {
            bridge.dispatch('engine', data + ' World!'); 
            process.exit(-1);
        });    `
});

engine.on('engine', function (data) {

    // isolated regions
    console.log(data); // Hello World!
    
    // open a file and run the script inside
    try {
        const scriptcode = fs.readFileSync("./script.js", 'utf8'); // 'utf8' specifies the encoding
        
        //console.log(scriptcode);
        let ret = eval(scriptcode);
...
engine.dispatch('engine', 'HelloOK ');